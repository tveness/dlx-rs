...
/// Implements the linked lists in `elements`
///
/// `elements` is structured in the following way
/// `
/// i0  ⟷ i1  ⟷ i2 ⟷ i3 ⟷ i4
///        ⥯      ⥯     ⥯     ⥯   s0
///        ⦿      ⦿     ⥯     ⥯   s1
...
/// `
///
/// where all of the spacers s0,..., also form a doubly circularly linked list.
/// i0 is the root node for the items (always there in order to traverse the list)
/// s0 is the root node for the spacers (always there in order to traverse options)
pub struct Solver {
    elements: Vec<Box<dyn Link>>,
    items: Index,
    pub options: HashMap<Index, Vec<Index>>,
    l: usize,
    sol_vec: Vec<Index>,
    yielding: bool,
    idx: Index,
...
impl Solver {
    ...
    pub fn add_option(&mut self, option: &[Index], name: &str) {
